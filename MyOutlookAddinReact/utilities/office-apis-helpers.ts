...
export const writeFileNamesToWorksheet = async (result: AxiosResponse, displayError: (x: string) => void) => {
  try {
  ...
  //   const data = [[result.data.value[0].name], [result.data.value[1].name], [result.data.value[2].name]];
...
export const signInO365 = async (
  ...
  const processLoginMessage = (args: { message: string; origin: string } ) => {
    ...
    if (messageFromDialog.status === "success") {
      ...
      setState({ authStatus: "loggedIn", headerMessage: "Get Data" });
    } else {
...
export const logoutFromO365 = async (setState: (x: AppState) => void, displayError: (x: string) => void) => {
